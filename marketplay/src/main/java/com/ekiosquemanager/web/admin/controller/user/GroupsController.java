...
import com.ekiosquemanager.core.utils.ajax.AjaxPageableResponse;
...
public class GroupsController {
	...
	private List<Map> groupList = new ArrayList<Map>();
	...
	public void populateGroupList(){
		...
		for(Group group : groups) {
			...
			entry.put("type", group.getGroupType());

			StringBuilder key = new StringBuilder().append("security.group.description.").append(group.getGroupName());
			try {
			
				String message =  messages.getMessage(key.toString(), Locale.ENGLISH);
	...
	@PreAuthorize("hasRole('STORE_ADMIN')")
	@RequestMapping(value = {"/admin/users/groups.html"}, method = RequestMethod.GET)
	public String usersGroups(HttpServletRequest request,
...
//		return returnString;
		model.addAttribute("groupsJSON", getGroupList());
		return "admin-user-groups";
	}
