...
import com.ekiosquemanager.core.business.reference.country.service.CountryService;
...
import com.ekiosquemanager.core.utils.ajax.AjaxResponse;
import com.ekiosquemanager.web.admin.controller.ControllerConstants;
import com.ekiosquemanager.web.admin.entity.security.Password;
import com.ekiosquemanager.web.admin.entity.userpassword.UserReset;
import com.ekiosquemanager.web.admin.entity.web.Menu;
import com.ekiosquemanager.web.admin.security.SecurityQuestion;
import com.ekiosquemanager.web.constants.Constants;
import com.ekiosquemanager.web.constants.EmailConstants;
import com.ekiosquemanager.web.utils.EmailUtils;
//import com.ekiosquemanager.web.utils.FilePathUtils;
import com.ekiosquemanager.web.utils.LabelUtils;
import com.ekiosquemanager.web.utils.LocaleUtils;
import com.ekiosquemanager.web.utils.UserUtils;
...
//				templateTokens.put(EmailConstants.EMAIL_ADMIN_URL, FilePathUtils.buildAdminUri(store, request));
	...
	@RequestMapping(value="/admin/users/resetPasswordApplicationJson.html", method=RequestMethod.POST, produces="application/json")
	public @ResponseBody String resetPasswordApplicationJson(HttpServletRequest request, HttpServletResponse response, Locale locale) {
	...
	//password reset functionality  ---  Sajid Shajahan  
		...
		public @ResponseBody String resetPassword(Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) {
			...
			try {
					if(!StringUtils.isBlank(userName)){
							...
							if(dbUser==null) {
								resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
								resp.setStatusMessage(messages.getMessage("message.username.notfound", locale));
								model.addAttribute("alertUser", messages.getMessage("message.username.notfound", locale));
								return resp.toJSONString();
							}
					...
					}else
					{
							resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
							resp.setStatusMessage(messages.getMessage("User.resetPassword.Error", locale));
							model.addAttribute("alertUser", messages.getMessage("User.resetPassword.Error", locale));
							...
							resp.setStatusMessage(messages.getMessage("User.resetPassword.Error", locale));
							model.addAttribute("alertUser", messages.getMessage("User.resetPassword.Error", locale));
							model.addAttribute("response", resp.toJSONString());
							return resp.toJSONString();
				}
	...
	//password reset functionality  ---  Sajid Shajahan
	@RequestMapping(value="/admin/users/resetPasswordSecurityQtnApplicationJson.html", method=RequestMethod.POST, produces="application/json")
	public @ResponseBody String resetPasswordSecurityQtnApplicationJson(@ModelAttribute(value="userReset") UserReset userReset,HttpServletRequest request, HttpServletResponse response, Locale locale) {
	...
	//password reset functionality  ---  Sajid Shajahan
		...
		public @ResponseBody String resetPasswordSecurityQtn(@ModelAttribute(value="userReset") UserReset userReset, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) {
			...
			try {
				...
				if(dbUser!= null){
					...
					if(dbUser.getAnswer1().equals(answer1.trim()) && dbUser.getAnswer2().equals(answer2.trim()) && dbUser.getAnswer3().equals(answer3.trim())){
						...
						try {
							...
							LOGGER.error("Cannot send email to user",e);
							msgEmail = "Cannot send email to user";
						}
						
						resp.setStatus(AjaxResponse.RESPONSE_OPERATION_COMPLETED);
						resp.setStatusMessage(messages.getMessage("User.resetPassword.resetSuccess", locale));
						model.addAttribute("alertSucess", messages.getMessage("User.resetPassword.resetSuccess", locale) + (StringUtils.isNotBlank(msgEmail) ? (" " + msgEmail) : ""));
					}
					else{
						  resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
						  resp.setStatusMessage(messages.getMessage("User.resetPassword.wrongSecurityQtn", locale));
						  model.addAttribute("alertError", messages.getMessage("User.resetPassword.wrongSecurityQtn", locale));
					  }
				  }else{
					  resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
					  resp.setStatusMessage(messages.getMessage("User.resetPassword.userNotFound", locale));
					  model.addAttribute("alertError", messages.getMessage("User.resetPassword.userNotFound", locale));
				  }
				...
				resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
				resp.setStatusMessage(messages.getMessage("User.resetPassword.Error", locale));
				model.addAttribute("alertError", messages.getMessage("User.resetPassword.Error", locale));
			} catch (NullPointerException e) {
				e.printStackTrace();
				resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
				resp.setStatusMessage(messages.getMessage("User.resetPassword.wrongSecurityQtn", locale));
